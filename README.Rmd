...
my_br = function(ems = 0) {
  span(style = gsub("VALUE", ems, "display: block; margin-bottom: VALUEem"))
}

indent_title = "margin-left: 40px;"
indent_content = "margin-left: 80px;"
...
article_details = function(key, open_access = FALSE, code_repo = NULL, code_doi = NULL) {
  ...
  if (!is.null(code_repo)) {
    repo = p(strong("Repository:"), a(href = paste0("https://github.com/", code_repo), code_repo), style = indent_content)
  } else {
    repo = p(strong("Repository:"), "Not Available", style = indent_content)
  }
  
  if (!is.null(code_doi)) {
    archive = p(strong("Archive:"), build_doi_badge(code_doi), style = indent_content)
  } else {
    archive = p(strong("Archive:"), "Not Available", style = indent_content)
  }
  ...
  tags$details(
    ...
    p(book_img, strong(em("JOURNAL")), style = indent_title),
    my_br(-0.9),
    p(build_journal_name(bib_data), build_journal_info(bib_data), br(), build_doi_badge(bib_data$doi), style = indent_content),
    my_br(-0.5),
    
    p(users_img, strong(em("AUTHORS")), style = indent_title),
    my_br(-0.9),
    p(HTML(my_name_strong(bib_data$authors)), style = indent_content),
    my_br(-0.5),
    
    p(book_reader_img, strong(em("ABSTRACT")), style = indent_title),
    my_br(-0.9),
    p(HTML(my_em(bib_data$abstract)), align = "justify", style = indent_content),
    my_br(-0.5),
    
    p(code_img, strong(em("CODE/DATA")), style = indent_title),
    my_br(-0.9),
    repo,
    my_br(-1),
    archive,
    ...
    p(comment_img, strong(em("CITATIONS")), style = indent_title),
    my_br(-0.9),
    p(strong("Crossref: "), bib_data$cites_crossref, style = indent_content),
    my_br(-1),
    p(strong("Google Scholar: "), bib_data$cites_gscholar, style = indent_content),
    my_br(-0.5),
    p()
  )
}
...
```{r footnote}
footer_style = "text-align: right; font-size: 12px;"

tags$footer(
  em("This file was created by knitting", a("README.Rmd", href = "https://github.com/bstaton1/bstaton1/blob/master/README.Rmd", ".")),
  style = footer_style
)
...
tags$footer(
  em(
    ...
    a("RefManageR", href = "https://docs.ropensci.org/RefManageR/"), "(publication metadata) and",
    a("rcrossref", href = "https://docs.ropensci.org/rcrossref/"), "and", a("scholar", href = "https://CRAN.R-project.org/package=scholar"), "(citation counts)."),
  style = footer_style
)

tags$footer(
  em(
    "R packages", a("htmltools", href = "https://rstudio.github.io/htmltools/"), "and", a("stringr", href = ""), "also used heavily."),
  style = footer_style
)

tags$footer(em("Updated", Sys.Date()), style = footer_style)
```
